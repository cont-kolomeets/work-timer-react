...
var TimeController = {
  ...
  _provideCurrentDayPath: function () {
    ...
    var year = (this.years[this._currentYear] =
      this.years[this._currentYear] || {}); // provide a year
    var months = (year.months = year.months || {});

    var month = (months[this._currentMonth] = months[this._currentMonth] || {}); // provide a month
    month.days = month.days || {}; // provide days hash
    // TODO: update timeNeeded
    month.timeNeeded =
      month.timeNeeded ||
      TimeConverter.partsToTotal({
        h: 8 * 20,
      });
  ...
  _saveCurrentData: function () {
    if (this._currentDay === -1) return;

    const m = this.years[this._currentYear].months[this._currentMonth];

    var dayInfo = (m.days[this._currentDay] = _toDayInfo(
  ...
  getCurrentMonthTimeList: function (daysOnly) {
    ...
    for (var id in month.days) createDayDataObject(id);
